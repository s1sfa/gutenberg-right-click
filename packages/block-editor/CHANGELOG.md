...
-   Added a right-click context menu inside the editor canvas. Items include `Blocks ▸` (Add block above…, Add block below…, Duplicate, Split block ▸ — which lists every block type the highlighted text can transform into via `getPossibleBlockTransformations` and replaces the source block with up to three new blocks, trimming whitespace at the joins), inline `Copy Text` / `Copy Blocks` / `Paste` / `Paste as plain text` / `Paste as new block ▸`, `Tools ▸ Check spelling` (LanguageTool inline replacements) and `Tools ▸ Edit as HTML`, plus `Delete Block` / `Delete Blocks` (labels switch with the target). Paragraph blocks gain inline `Bold` / `Italic` / `Inline code` that toggle the same `RichText` formats the toolbar uses, and `Font color…` which opens a tabbed Text / Background color picker popover mirroring the format library's `InlineColorUI` using `block-editor` primitives. The menu dismisses when the user left-clicks or starts typing inside the canvas while it is open. Group / Lock / Rename / Visibility / Create pattern / Paste styles remain in the block kebab menu only.
-   New `BlockContextMenuControls` slot for extending the right-click menu. Fills receive `clientIds`, `range`, `ownerDocument`, `selectionText`, `view`, `setView`, `openColorPicker`, and `onClose`. Each fill supplies its own `MenuGroup`(s), and can push its own submenu by setting `view` to a fill-namespaced name — the parent then renders the slot exclusively so the fill owns the screen. Items that should appear in both the right-click and kebab menus continue to register against `BlockSettingsMenuControls`. The table block uses this slot to add `Row edit ▸`, `Column edit ▸`, `Alignment ▸`, `Show Header Row`, and `Show Footer Row`, with a check icon on the active alignment / section.
